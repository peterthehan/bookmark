const blacklist = ['chrome://bookmarks', 'chrome://newtab'];
...
function goToBookmarksManager(tabs, filtered, result) {
  if (tabs.length === 0) {
    chrome.tabs.create(createProperties(result.id, true), _ =>
      removeTabs(filtered)
    );
    return;
  }
  ...
  chrome.tabs.update(firstTab.id, createProperties(result.id, true), _ => {
    chrome.windows.update(firstTab.windowId, { focused: true }, _ =>
      removeTabs(filtered)
    );
  });
...
chrome.browserAction.onClicked.addListener(_ => {
  chrome.tabs.query({ pinned: false, currentWindow: true }, tabs => {
    ...
    chrome.bookmarks.create(
      { index: 0, title: new Date().toLocaleString() },
      result => {
        ...
        chrome.tabs.query({ url: 'chrome://bookmarks/*' }, tabs =>
          goToBookmarksManager(tabs, filtered, result)
        );
      }
    );
  });
